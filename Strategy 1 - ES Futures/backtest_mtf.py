"""
Strategy 2: Multi-Timeframe Filter
60-min chart sets direction bias (HA EMA + RSI).
15-min chart entries only when aligned with 60-min bias.
...
from config import RSI_BUY_THRESHOLD, RSI_SELL_THRESHOLD, INITIAL_CAPITAL, ES_POINT_VALUE
...
def get_60m_bias(df_60m):
    ...
    For each 60-min bar, determine directional bias:
      ...
      -1 = sell bias (HA close < EMA low AND RSI < sell threshold)
       0 = neutral
    Returns a Series indexed by 60-min bar timestamps.
    """
...
def align_bias_to_15m(bias_60m, df_15m):
    """
    Map 60-min bias to each 15-min bar.
    Each 15-min bar gets the bias of the most recent completed 60-min bar.
    """
    aligned = pd.Series(0, index=df_15m.index, dtype=int)
    bias_times = bias_60m.index
    for i, ts in enumerate(df_15m.index):
        # Find most recent 60m bar at or before this 15m bar
        mask = bias_times <= ts
        if mask.any():
...
def simulate(df_15m, bias_15m, atr_stop_mult, atr_target_mult, s_start_h, s_start_m, s_end_h, s_end_m):
    """Run strategy: only enter when 15m signal aligns with 60m bias. P&L in points."""
    position = 0
    ...
    for i in range(1, len(df_15m)):
        ...
        # Entry: 15m signal must ALIGN with 60m bias, and be in session
        if position == 0 and in_session:
            # 15m buy signal AND 60m bias is bullish
            ...
            # 15m sell signal AND 60m bias is bearish
...
def main():
    print("Fetching 5m data, resampling to 15m and 60m...")
    df_5m = fetch_es_data()
    df_15m_raw = resample(df_5m, "15min")
    df_60m_raw = resample(df_5m, "60min")
    ...
    # Compute 60m bias and map to 15m
    bias_60m = get_60m_bias(df_60m)
    bias_15m = align_bias_to_15m(bias_60m, df_15m)

    buy_bars = (bias_15m == 1).sum()
    sell_bars = (bias_15m == -1).sum()
    neutral_bars = (bias_15m == 0).sum()
    print(f"60m bias distribution: BUY={buy_bars} SELL={sell_bars} NEUTRAL={neutral_bars}")
    ...
    for sess_name, sh, sm, eh, em in SESSIONS:
        for stop in STOP_RANGE:
            for target in TARGET_RANGE:
                eq, tr = simulate(df_15m, bias_15m, stop, target, sh, sm, eh, em)
    ...
    if len(profitable) > 0:
        print("=" * 110)
        print(f"  MTF PROFITABLE COMBOS (PF >= 1.0) — {len(profitable)} found!  [POINTS]")
        print("=" * 110)
    ...
    html = f"""<!DOCTYPE html>
<html><head><title>ES MTF (60m+15m) Grid — {len(tbl)} combos</title>
<style>
...
<h1>Strategy 2: Multi-Timeframe — 60m Bias + 15m Entries</h1>
<h2>60m sets direction, 15m aligned entries only | RSI {RSI_BUY_THRESHOLD}/{RSI_SELL_THRESHOLD} | $50k account @ $50/pt</h2>
<h2>{len(tbl)} combos | 15m bars: {len(df_15m)} | 60m bias: BUY={buy_bars} SELL={sell_bars} NEUTRAL={neutral_bars}</h2>
<div class="summary">
...
{styled.to_html()}
    ...
    output = "backtest_mtf_chart.html"
